...
public abstract class AbstractMessageHandler<T extends IMessage> implements IMessageHandler <T, IMessage> {
	...
	*/
	...
	return handleClientMessage(TurtleMain.proxy.getPlayerEntity(ctx), message, ctx);
	} else {
	// server side proxy will return the server side EntityPlayer
	return handleServerMessage(TurtleMain.proxy.getPlayerEntity(ctx), message, ctx);
	}
	}
	}
